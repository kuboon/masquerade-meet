...
この経路は実際のミーティングでしか目視できず（Realtime の認証情報が要る）、上記のループもそれで見逃されました。`/dev/still-image` はルームと同じ `outgoingVideoTrack$` を購読して再生するだけの開発用ページで、トラックが出ているか、フレームが実際に届いているかを認証情報なしに確認できます。e2e もここを叩いています。
