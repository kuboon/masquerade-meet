 ...
 * `packages/character-set`, which is the contract this app publishes: a
 * stranger writing a set on their own site has to know what `size: -0.95`
 * means and what will get their set refused, and there must be exactly one
 * answer to both. Re-exported here so that everything in the app can keep
 * saying `~/utils/characters`.
 *
...
import type { VoiceParams } from '../../packages/character-set/mod.ts'

export { isDisguised, VOICE_RANGE } from '../../packages/character-set/check.ts'
export type {
	Character,
	CharacterSet,
	VoiceParams,
} from '../../packages/character-set/mod.ts'
